...
export const IS_SINGLE_COLUMN_LAYOUT =
  window.innerWidth <= MOBILE_SCREEN_MAX_WIDTH ||
  (window.innerWidth <= MOBILE_SCREEN_LANDSCAPE_MAX_WIDTH &&
    window.innerHeight <= MOBILE_SCREEN_LANDSCAPE_MAX_HEIGHT);
// Special layout, 1 column while chat opened, 2 columns while collapsed
export const IS_TABLET_COLUMN_LAYOUT =
...
export const IS_SMOOTH_SCROLL_SUPPORTED =
...
function isLastEmojiVersionSupported() {
  ...
  return (
    Math.abs(newEmojiWidth - legacyEmojiWidth) <
    ALLOWABLE_CALCULATION_ERROR_SIZE
  );
}
