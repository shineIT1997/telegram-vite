...
const FILTERED_ATTRIBUTES = new Set([
  "key",
  "ref",
  "teactFastList",
  "teactOrderKey",
...
function render($element: VirtualElement | undefined, parentEl: HTMLElement) {
  ...
  const $newElement = renderWithVirtual(
    parentEl,
    $head.children[0],
    $element,
    $head,
...
function renderWithVirtual<T extends VirtualElement | undefined>(
  ...
  const isCurrentFragment =
    $current && !isCurrentComponent && isFragmentElement($current);
  ...
  if (
    !skipComponentUpdate &&
    isNewComponent &&
    ($new as VirtualElementComponent).componentInstance.isMounted
  ) {
    setupComponentUpdateListener(
      parentEl,
      $new as VirtualElementComponent,
      $parent,
      index
  ...
  if (DEBUG && $new) {
    ...
    if (
      newTarget &&
      (!$current || ("target" in $current && newTarget !== $current.target))
    ) {
  ...
  if (!$current && $new) {
    ...
    if (hasElementChanged($current, $new)) {
      ...
      if (isComponent || isFragment) {
        ($new as VirtualElementComponent | VirtualElementFragment).children =
          renderChildren(
            $current,
            $new as VirtualElementComponent | VirtualElementFragment,
            parentEl,
...
function updateComponent(
  $current: VirtualElementComponent,
  $new: VirtualElementComponent
) {
...
function renderChildren(
  ...
  const fragment =
    newChildrenLength > currentChildrenLength
      ? document.createDocumentFragment()
      : undefined;
  const lastCurrentChild = $current.children[currentChildrenLength - 1];
  const fragmentNextSibling =
    nextSibling ||
    (newChildrenLength > currentChildrenLength && lastCurrentChild
      ? getNextSibling(lastCurrentChild)
      : undefined);
...
function renderFastListChildren(
  ...
  const remainingByKey = $current.children.reduce((acc, $currentChild, i) => {
    ...
    if (isKeyPresent && !newKeys.has(key)) {
      ...
      const newChildKey =
        $newChild && "props" in $newChild ? $newChild.props.key : undefined;
    ...
    acc[key] = {
      ...
      orderKey:
        "props" in $currentChild
          ? $currentChild.props.teactOrderKey
          : undefined,
    };
    return acc;
  ...
  $new.children.forEach(($newChild, i) => {
    const key =
      "props" in $newChild ? $newChild.props.key : `${INDEX_KEY_PREFIX}${i}`;
    ...
    if (fragmentElements) {
      newChildren = newChildren.concat(
        renderFragment(fragmentElements, fragmentIndex!, currentEl, $new)
      );
    ...
    const newOrderKey =
      "props" in $newChild ? $newChild.props.teactOrderKey : undefined;
    // That is indicated by a changed `teactOrderKey` value
    const shouldMoveNode =
      currentChildInfo.index !== currentPreservedIndex &&
      currentChildInfo.orderKey !== newOrderKey;
    const isMovingDown =
      shouldMoveNode && currentPreservedIndex > currentChildInfo.index;
    ...
    newChildren.push(
      renderWithVirtual(
        currentEl,
        currentChildInfo.$element,
        $newChild,
        $new,
        i,
        {
          // `+ 1` is needed because before moving down the node still takes place above
          nextSibling: shouldMoveNode
            ? currentEl.childNodes[isMovingDown ? i + 1 : i]
            : undefined,
        }
      )
    );
...
function processControlled(tag: string, props: AnyLiteral) {
  ...
  const isControlled =
    (isValueControlled || isCheckedControlled) &&
    CONTROLLABLE_TAGS.includes(tag.toUpperCase());
  if (!isControlled) {
    return;
  }
...
function updateAttributes(
  $current: VirtualElementTag,
  $new: VirtualElementTag,
  element: HTMLElement
) {
  ...
  if (key === "className") {
    ...
    if (inputEl.value !== value) {
      ...
      if (selectionStateJson) {
        const { selectionStart, selectionEnd, isCaretAtEnd } = JSON.parse(
          selectionStateJson
        ) as SelectionState;
...
// eslint-disable-next-line @typescript-eslint/naming-convention
function DEBUG_addToVirtualTreeSize(
  $current: VirtualElementParent | VirtualDomHead
) {
...
function DEBUG_checkKeyUniqueness(children: VirtualElementChildren) {
  ...
  if (
    firstChild &&
    "props" in firstChild &&
    firstChild.props.key !== undefined
  ) {
